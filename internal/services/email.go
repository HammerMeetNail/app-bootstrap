...
func NewEmailService(cfg *config.EmailConfig, db DBConn) *EmailService {
	...
	switch cfg.Provider {
	case "resend":
		provider = NewResendProvider(cfg.ResendAPIKey, cfg.FromName, cfg.FromAddress)
	case "smtp":
		provider = NewSMTPProvider(cfg.SMTPHost, cfg.SMTPPort, cfg.FromName, cfg.FromAddress)
	default:
		provider = NewConsoleProvider(cfg.FromName, cfg.FromAddress)
	}
...
func (s *EmailService) SendVerificationEmail(ctx context.Context, userID uuid.UUID, email string) error {
	...
	return s.provider.Send(ctx, &Email{
		To:      email,
		Subject: fmt.Sprintf("Verify your %s account", s.fromName),
		HTML:    html,
		Text:    text,
	})
}
...
func (s *EmailService) SendMagicLinkEmail(ctx context.Context, email string) error {
	...
	return s.provider.Send(ctx, &Email{
		To:      email,
		Subject: fmt.Sprintf("Your %s login link", s.fromName),
		HTML:    html,
		Text:    text,
	})
}
...
func (s *EmailService) SendPasswordResetEmail(ctx context.Context, userID uuid.UUID, email string) error {
	...
	return s.provider.Send(ctx, &Email{
		To:      email,
		Subject: fmt.Sprintf("Reset your %s password", s.fromName),
		HTML:    html,
		Text:    text,
	})
}
...
<body style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px;">
  <h1 style="color: #333; font-size: 24px;">Welcome to %s!</h1>
...
</html>`, s.fromName, verifyURL, verifyURL, s.fromName)
...
--
%s`, s.fromName, verifyURL, s.fromName)
...
<body style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px;">
  <h1 style="color: #333; font-size: 24px;">Sign in to %s</h1>
...
</html>`, s.fromName, loginURL, loginURL, s.fromName)
...
--
%s`, s.fromName, loginURL, s.fromName)
...
</html>`, resetURL, resetURL, s.fromName)
...
func NewResendProvider(apiKey, fromName, fromAddress string) *ResendProvider {
...
func NewSMTPProvider(host string, port int, fromName, fromAddress string) *SMTPProvider {
	return &SMTPProvider{
		host:        host,
		port:        port,
		fromName:    fromName,
...
func (p *SMTPProvider) Send(ctx context.Context, email *Email) error {
	...
	fromHeader := fmt.Sprintf("%s <%s>", p.fromName, p.fromAddress)
	...
	err := smtp.SendMail(addr, nil, p.fromAddress, []string{email.To}, buf.Bytes())
	if err != nil {
...
func NewConsoleProvider(fromName, fromAddress string) *ConsoleProvider {
	_ = fromName
	_ = fromAddress
