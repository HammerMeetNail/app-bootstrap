...
VERSION="${version}" perl -0777 -i -pe 's/(id="app-version">)v\d+\.\d+\.\d+/$1."v".$ENV{VERSION}/ge' "${index_file}"
VERSION="${version}" perl -i -pe 's/^  version: \d+\.\d+\.\d+$/  version: $ENV{VERSION}/' "${openapi_file}"

version_regex="${version//./\\.}"
tag_regex="v${version_regex}"

if ! grep -Eq "id=\"app-version\">${tag_regex}" "${index_file}"; then
  echo "Error: failed to update ${index_file} (expected footer to show ${tag})" 1>&2
  exit 1
fi
